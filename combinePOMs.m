function POM_com = combinePOMs(POMs)
...
if num_pom<2
    ...
    for j=2:num_pom
        ...
        idx = lob(lob>0);
        if ~isempty(idx)
            % POM_com(idx,2) = min(POM_com(idx,2)+POM_temp(lia,2),1);
            POM_com(idx,2) = min(POM_com(idx,2),POM_temp(lia,2));
            % we do not use maxout in this case
